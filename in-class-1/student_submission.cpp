...
void preprocess_mapping(const uint8_t* keys, const uint8_t* values, uint8_t* map) {
    // Initialize map for direct access
 ...
 */
void decrypt_message(uint8_t *decryptedMessage, const uint8_t *map)
{
    for (unsigned int i = 0; i < STRING_LEN; ++i)
    {
        decryptedMessage[i] = map[decryptedMessage[i]];
    }
}
...
int main()
{
    ...
    */
    uint8_t keys[UNIQUE_CHARACTERS];
    ...
    */
    uint8_t values[UNIQUE_CHARACTERS];

    uint8_t map[UNIQUE_CHARACTERS]; // Mapping array
    ...
    */
    ...
    preprocess_mapping(keys, values, map);

    for (int i = 0; i < NUM_ITERATIONS; i++)
    {
        decrypt_message(decryptedMessage, map);
    }
