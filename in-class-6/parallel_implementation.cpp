...
int main(int argc, char **argv)
{
    ...
    auto *old_wave_map   = new float [MAP_HEIGHT * MAP_WIDTH];
    auto *new_wave_map   = new float [MAP_HEIGHT * MAP_WIDTH];
    auto *highest_result = new float[ITER_NUM];
    ...
    for (int i = 0; i < ITER_NUM; i++)
    {
        ...
        if (rank == 0)
        {
            // send the lowest row in block to process with rank 1
            ...
            // send the uppermost row in block to process with rank size-1
