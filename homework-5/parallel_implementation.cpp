...
bool findPathWithExhaustiveSearch(ProblemData &problemData, int timestep, std::vector<Position2D> &pathOutput) {
    ...
    #pragma omp parallel for collapse(2) reduction(||:foundPath) reduction(+:numPossiblePositions)
    for (int x = 0; x < MAP_SIZE; ++x) {
        for (int y = 0; y < MAP_SIZE; ++y) {
...
int main(int argc, char *argv[]) {
    ...
    std::cout << std::dec << "Solve Problem time: " << time_in_milliseconds << " milliseconds" << std::endl;
#endif
